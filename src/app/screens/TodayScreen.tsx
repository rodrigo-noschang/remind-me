...
export function TodayScreen() {
	const { completeTodayTask, error, isLoading, reopenTodayTask, todayTasks } =
		useTasks();
	const [updatingTaskId, setUpdatingTaskId] = useState<string | null>(null);
	const { completedTasks, pendingTasks } = useMemo(
		() => ({
			completedTasks: sortByCompletionTime(
				todayTasks.filter((item) => Boolean(item.occurrence?.completedAtUtc)),
			),
			pendingTasks: sortByReminderTime(
				todayTasks.filter((item) => !item.occurrence?.completedAtUtc),
			),
		}),
		[todayTasks],
	);
...
function compareByReminderTime(left: TaskWithRule, right: TaskWithRule) {
	const timeComparison = left.reminderRule.localTime.localeCompare(
		right.reminderRule.localTime,
	);
